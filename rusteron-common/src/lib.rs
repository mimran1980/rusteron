...
///
/// It handles initialization and cleanup of the resource and ensures that resources
...
impl<T> Drop for ManagedCResource<T> {
    fn drop(&mut self) {
        // Ensure the cleanup function is called when the resource is dropped.
        let _ = self.close(); // Ignore errors during automatic drop to avoid panics.
    }
}
